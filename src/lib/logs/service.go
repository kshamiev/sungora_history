...
func logsMessageCalculate(code int, message string, level uint8) string {
	// временная отметка
	var prefix = lib.Time.Label()

	// формируем
	var logLine = fmt.Sprintf("%s\t%s[%d]", prefix, logsLevel[level], code)
	if cfg.Debug == true {
		logLine += `[debug]`
	}
	logLine += "\t" + message
	...
	if cfg.Debug == true && cfg.DebugDetail >= 1 {
