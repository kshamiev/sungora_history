...
//                  'vendor/pace/pace.min.js'
        ...
        copy: {
            ...
            build: {
                files: [
                    //{ expand: true, cwd: '../../api/v1', dest: '<%= distdir %>/users/', src: ['login', 'logout', 'registration'] },
                    //{ expand: true, cwd: '../../api/v1', dest: '<%= distdir %>/api/users/', src: ['current-user', 'check-cookie'] },
                    //{ expand: true, cwd: '../../api/v1', dest: '<%= distdir %>/', src: ['captcha'] },
                    { expand: true, cwd: 'src', dest: '<%= distdir %>/', src: 'index.html'}
                ]
            },
            //release: {
               //files: [
                  //{ expand: true, cwd: 'src/user/', dest: '<%= releaseDir %>/', src: '*.html'},
